...
prot.exp <- as.data.frame(readxl::read_excel("data/Cohort3_proteomics.xlsx")) # Open the data
rownames(prot.exp) <- prot.exp$Accession

# filtering by number valid values

perc.VV <- round((30*77)/100,0) # defining the 30% of the samples (considering 77 samples)

prot.exp1 <- subset(prot.exp,prot.exp$`Valid Values`>perc.VV)
hist(prot.exp1$`Valid Values`)
...
hist(prot.exp_short1$NT044, breaks = 100)
...
hist(prot.exp_short1.imp$NT044, breaks = 100)
...
write.csv(easier_derived_scores_MM,'output/4to_easier_derived_scores_prot_77samples.csv')
